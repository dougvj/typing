...
void drawBuffer(char* buffer, int start, int offset, int rows, int columns, int attribute) {
...
//text
void drawCurrentBuffer(char* buffer, int start, int offset, int rows, int columns) {
...
//details
void drawCurrentInput(char* input, int start, int offset, int rows, int columns) {
...
//This function adds a character to the input buffer output and paints it either
//blue or red depending on whether wrong is set correctly. The offset is row 
//offset from center (same as above). Pos is the column offset to draw it at.
//As always to calculate the proper distance from center, the number of rows
//and columns are needed
void addCharacter(int wrong, char chr, int offset, int pos, int rows, int columns) {
...
void redraw(char* buffer, char* input, int spos, int cpos, int rows, int columns) {
...
void print_stats(int chr, int wd, double time, int incor) {
    attron(COLOR_PAIR(4));
    printw("cps: %.3hhf cpm: %.3hhf wps: %.3hhf wpm: %.3hhf accuracy: %2.1f", chr/time, chr/time * 60, wd / time, wd / time * 60, 100 * (chr - incor) / (float)chr);
...
void typing(FILE* desc) {
    ...
    endwin();
    time = get_time() - time;
    printf("characters per second: %.3hhf\ncharacters per minute: %.3hhf\nwords per second: %.3hhf\nwords per minute: %.3hhf\naccuracy:%2.1f\n", chr/time, chr/time * 60, wd / time, wd / time * 60, 100 * (chr - incor) / (float)chr);
